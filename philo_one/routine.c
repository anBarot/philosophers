...
/*   Updated: 2021/05/18 14:17:01 by abarot           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*monitor_routine(void)
{
	...
	while (g_phi.dead == false)
	{
		...
		while (i < g_phi.philo_nb)
		{
			...
			if ((time - g_phi.phi_unit[i].last_time_eat) >= g_phi.tt_die)
			{
				g_phi.dead = true;
				pthread_mutex_unlock(&(g_phi.phi_unit[i].read_time_mutex));
				display_act(i + 1, S_DIE, time);
				return (NULL);
			}
			pthread_mutex_unlock(&(g_phi.phi_unit[i].read_time_mutex));
			i++;
...
void	taking_forks(t_thread *philo)
{
	...
	(g_phi.dead == false) ? display_act(philo->phi_nb + 1, S_FORK, get_time()) : 0;
	(philo->phi_nb % 2) ? pthread_mutex_lock(
...
void	eating_routine(t_thread *philo)
{
	...
	(g_phi.dead == false) ? display_act(philo->phi_nb + 1, S_EAT, time) : 0;
...
void	*philo_routine(t_thread *philo)
{
	...
	while (g_phi.dead == false)
	{
		...
		{
			(g_phi.dead == false) ? display_act(philo->phi_nb, S_REACHED, get_time()) : 0;
